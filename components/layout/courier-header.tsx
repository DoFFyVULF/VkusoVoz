...
const NAV = [
  { label: "Заказы", href: "/courier" },
  { label: "История", href: "/courier?tab=history" },
] as const
...
export function CourierHeader() {
  ...
  const initial = user?.name?.charAt(0)?.toUpperCase() ?? "К"
  ...
  return (
    <header className="sticky top-0 z-40 flex h-14 w-full items-center border-b bg-card px-4 lg:px-6">
      <div className="flex w-full items-center gap-4">
        <Link href="/courier" className="flex items-center gap-2 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <span className="flex size-8 items-center justify-center rounded-lg bg-primary text-primary-foreground">
            <Bike className="size-4" />
          </span>
          <span className="hidden text-sm font-bold tracking-tight sm:inline">ВкусоВоз</span>
          <span className="hidden text-sm font-medium text-muted-foreground sm:inline">· Курьер</span>
          <span className="inline text-sm font-bold sm:hidden">ВВ · Курьер</span>
        </Link>

        <nav aria-label="Навигация курьера" className="hidden items-center gap-1 lg:flex ml-6">
          {NAV.map((t) => (
            <Link
              key={t.label}
              href={t.href}
        ...
        <div className="ml-auto flex items-center gap-2">
          <span className="hidden items-center gap-2 sm:flex">
            ...
            <span className="hidden flex-col lg:flex">
              <span className="max-w-[140px] truncate text-xs font-medium leading-none">{user?.name ?? "Курьер"}</span>
              <span className="inline-flex rounded-full bg-primary px-1.5 py-0.5 text-[10px] font-semibold leading-none text-primary-foreground">COURIER</span>
            </span>
          </span>
